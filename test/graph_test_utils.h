...
class graph_query {
  public:
	...
	explicit graph_query(const Recorder& recorder) : graph_query(&recorder, non_owning_pointers(recorder.get_all()), std::string()) {}
	...
	base_query predecessors() const {
		...
		for(const auto& maybe_predecessor : m_recorder->get_all()) {
	...
	base_query successors() const {
		...
		for(const auto& maybe_successor : m_recorder->get_all()) {
			if(std::any_of(m_recorder->get_dependencies().begin(), m_recorder->get_dependencies().end(), [&](const auto& dep) {
	...
	bool all_match(const Filters&... filters) const {
		...
		for(const Record* instr : m_result) {
			if(!matches<SpecificRecord>(*instr, filters...)) {
				...
				fmt::format_to(std::back_inserter(non_matching), "{}{}", gq_print_prefix<Recorder>, instr->id);
			}
		}
  ...
  private:
	...
	static bool matches(const Record& instr, const Filters&... filters) {
		return utils::isa<SpecificRecord>(&instr) && (Matcher<SpecificRecord>::matches(*utils::as<SpecificRecord>(&instr), filters) && ...);
	}
	...
	base_query union_with(const GraphQueries&... tail) const {
		...
		for(const auto& instr : m_recorder->get_all()) {
	...
	graph_query difference_with(const graph_query& other) const {
		...
		for(const auto& instr : m_result) {
			if(std::find(other.m_result.begin(), other.m_result.end(), instr) == other.m_result.end()) { difference_query.push_back(instr); }
		}
