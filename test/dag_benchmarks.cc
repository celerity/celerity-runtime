...
TEST_CASE("benchmark task handling", "[benchmark][group:task-graph]") {
	...
	// TODO drop extra section, simplify
...
struct task_manager_benchmark_context {
	...
	task_manager_benchmark_context() { tm.generate_epoch_task(epoch_action::none); }
...
struct command_graph_generator_benchmark_context : private task_manager::delegate {
	...
	explicit command_graph_generator_benchmark_context(const size_t num_nodes) : num_nodes(num_nodes) { tm.generate_epoch_task(epoch_action::none); }

	void task_available(const task* tsk) override { cggen.build_task(*tsk); }
...
struct scheduler_benchmark_context : private task_manager::delegate {
	...
	void task_available(const task* tsk) override { schdlr.notify_task_created(tsk); }
