...
void test_copy(celerity::distr_queue& q) {
	...
	const auto sr = celerity::subrange(truncate_id<Dims>({1, 2, 3}), truncate_range<Dims>({3, 4, 5}));
			...
			for(size_t k = 0; k < r3[0]; ++k) {
				for(size_t j = 0; j < r3[1]; ++j) {
					for(size_t i = 0; i < r3[2]; ++i) {
						const auto idx3 = celerity::id(k, j, i);
						const auto idx = truncate_id<Dims>(idx3);
						const auto is_in_sr = all_true(idx3 >= sr3.offset) && all_true(idx3 < sr3.offset + sr3.range);
						const auto linear_id = celerity::detail::get_linear_index(buf.get_range(), idx);
						if(is_in_sr) {
