...
namespace detail {
	...
	TEST_CASE_METHOD(test_utils::runtime_fixture, "accessors behave correctly for 0-dimensional master node kernels", "[accessor]") {
		...
		q.submit([&](handler& cgh) {
			auto a = buf_a.get_access<access_mode::read_write, target::host_task>(cgh, fixed<1>({0, 1}));
			cgh.host_task(on_master_node, [=] { ++a[0]; });
		});
		int out = 0;
		q.submit([&](handler& cgh) {
			auto a = buf_a.get_access<access_mode::read, target::host_task>(cgh, fixed<1>({0, 1}));
