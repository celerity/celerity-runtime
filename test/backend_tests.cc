...
struct host_or_device {
	...
	void malloc(const size_t count) {
		if(queue.has_value()) {
			...
			ptr = static_cast<T*>(std::malloc(count * sizeof(T))); // NOLINT cppcoreguidelines-no-malloc
		}
	}
...
TEMPLATE_TEST_CASE_SIG("memcpy_strided_device allows to copy between the same device, different devices on the same platform and between host and device",
	...
	src.template malloc<size_t>(cp.source_range.size());
	tgt.template malloc<size_t>(cp.target_range.size());

	src.run([ptr = src.ptr, cp = cp]() { write_linear_ids(ptr, cp.source_range); });
