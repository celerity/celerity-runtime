...
namespace detail {
	...
	TEST_CASE("task_manager does not create multiple dependencies between the same tasks", "[task_manager][task-graph]") {
		...
		SECTION("true dependencies") {
			...
			CHECK(tt.has_dependency(tid_b, tid_a));

			const auto its = tt.get_task(tid_a)->get_dependents();
		...
		SECTION("anti-dependencies") {
			...
			CHECK(tt.has_dependency(tid_b, tid_a, dependency_kind::anti_dep));
		...
		SECTION("true and anti-dependencies combined") {
			SECTION("if true is declared first") {
				...
				CHECK(tt.has_dependency(tid_b, tid_a));
				CHECK_FALSE(tt.has_dependency(tid_b, tid_a, dependency_kind::anti_dep));
	...
	TEST_CASE("task_manager respects range mapper results for finding dependencies", "[task_manager][task-graph]") {
		...
		CHECK(tt.has_dependency(tid_b, tid_a));
		CHECK(tt.has_dependency(tid_b, task_manager::initial_epoch_task)); // for read of the host-initialized part
		...
		CHECK_FALSE(tt.has_dependency(tid_c, tid_a));
		CHECK(tt.has_dependency(tid_c, task_manager::initial_epoch_task)); // for read of the host-initialized part
	}
	...
	TEST_CASE("task_manager correctly generates anti-dependencies", "[task_manager][task-graph]") {
		...
		REQUIRE(tt.has_dependency(tid_c, tid_a, dependency_kind::anti_dep));
		REQUIRE_FALSE(tt.has_dependency(tid_c, tid_b, dependency_kind::anti_dep));
		...
		REQUIRE_FALSE(tt.has_dependency(tid_d, tid_a, dependency_kind::anti_dep));
		REQUIRE(tt.has_dependency(tid_d, tid_b, dependency_kind::anti_dep));
	}
	...
	TEST_CASE("task_manager correctly handles host-initialized buffers", "[task_manager][task-graph]") {
		...
		CHECK(tt.has_dependency(tid_a, task_manager::initial_epoch_task));
		...
		CHECK_FALSE(tt.has_dependency(tid_b, task_manager::initial_epoch_task));

		const auto tid_c = test_utils::add_compute_task<class UKN(task_c)>(tt.tm, [&](handler& cgh) {
			host_init_buf.get_access<mode::discard_write>(cgh, fixed<1>{{0, 128}});
		});
		CHECK(tt.has_dependency(tid_c, tid_a, dependency_kind::anti_dep));
		const auto tid_d = test_utils::add_compute_task<class UKN(task_d)>(tt.tm, [&](handler& cgh) {
		...
		CHECK_FALSE(tt.has_dependency(tid_d, tid_b, dependency_kind::anti_dep));
	}
	...
	TEST_CASE("task_manager correctly handles dependencies for R/W modes", "[task_manager][task-graph]") {
		...
		for(const auto& mode_set : rw_mode_sets) {
			...
			REQUIRE(tt.has_dependency(tid_b, tid_a, dependency_kind::anti_dep));
		}
	}
	...
	TEST_CASE("task_manager handles all producer/consumer combinations correctly", "[task_manager][task-graph]") {
		...
		for(const auto& consumer_mode : detail::access::consumer_modes) {
			for(const auto& producer_mode : detail::access::producer_modes) {
				...
				CHECK(tt.has_dependency(tid_c, tid_b, pure_consumer || pure_producer ? dependency_kind::anti_dep : dependency_kind::true_dep));
			}
		}
	}
	...
	TEST_CASE("task_manager generates pseudo-dependencies for collective host tasks", "[task_manager][task-graph]") {
		...
		CHECK_FALSE(tt.has_any_dependency(tid_master, tid_collective_implicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_master, tid_collective_implicit_2));
		CHECK_FALSE(tt.has_any_dependency(tid_master, tid_collective_explicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_master, tid_collective_explicit_2));

		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_1, tid_master));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_1, tid_collective_implicit_2));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_1, tid_collective_explicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_1, tid_collective_explicit_2));

		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_2, tid_master));
		CHECK(tt.has_dependency(tid_collective_implicit_2, tid_collective_implicit_1, dependency_kind::true_dep));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_2, tid_collective_explicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_implicit_2, tid_collective_explicit_2));

		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_1, tid_master));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_1, tid_collective_implicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_1, tid_collective_implicit_2));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_1, tid_collective_explicit_2));

		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_2, tid_master));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_2, tid_collective_implicit_1));
		CHECK_FALSE(tt.has_any_dependency(tid_collective_explicit_2, tid_collective_implicit_2));
		CHECK(tt.has_dependency(tid_collective_explicit_2, tid_collective_explicit_1, dependency_kind::true_dep));
	}

	void check_path_length_and_front(test_utils::task_test_context& tt, int path_length, const std::unordered_set<task_id>& exec_front) {
		{
			INFO("path length");
			CHECK(task_manager_testspy::get_max_pseudo_critical_path_length(tt.tm) == path_length);
		}
		{
			...
			std::transform(exec_front.cbegin(), exec_front.cend(), std::inserter(task_exec_front, task_exec_front.begin()),
			    [&](const task_id tid) { return const_cast<task*>(tt.get_task(tid)); });
			CHECK(task_manager_testspy::get_execution_front(tt.tm) == task_exec_front);
		}
	}
	...
	TEST_CASE("task_manager keeps track of max pseudo critical path length and task front", "[task_manager][task-graph][task-front]") {
		...
		check_path_length_and_front(tt, 1, {tid_a}); // 1: we always depend on the initial epoch task

		const auto tid_b = test_utils::add_host_task(tt.tm, on_master_node, [&](handler& cgh) {
			buf_a.get_access<access_mode::read_write>(cgh, fixed<1>({0, 128}));
		});
		check_path_length_and_front(tt, 2, {tid_b});
		...
		check_path_length_and_front(tt, 3, {tid_c});

		const auto tid_d = test_utils::add_host_task(tt.tm, on_master_node, [&](handler& cgh) {});
		check_path_length_and_front(tt, 3, {tid_c, tid_d});
	}
	...
	TEST_CASE("task horizons update previous writer data structure", "[task_manager][task-graph][task-horizon]") {
...

		{
			...
			CHECK(region_map_a.get_region_values(make_region(0, 32)).front().second == tt.get_task(tid_1));
			CHECK(region_map_a.get_region_values(make_region(96, 128)).front().second == tt.get_task(tid_2));
			CHECK(region_map_a.get_region_values(make_region(32, 96)).front().second == tt.get_task(tid_4));
		}
...

		{
			...
			CHECK(region_map_a.get_region_values(make_region(64, 128)).front().second == tt.get_task(tid_9));
		}
	}
	...
	TEST_CASE("previous task horizon is used as last writer for host-initialized buffers", "[task_manager][task-graph][task-horizon]") {
		...
		task_id initial_last_writer_id = -1;
		{
			...
			const auto& deps = tt.get_task(tid)->get_dependencies();
			CHECK(std::distance(deps.begin(), deps.end()) == 1);
			initial_last_writer_id = deps.begin()->node->get_id();
		}
		CHECK(tt.has_task(initial_last_writer_id));
		...
		CHECK_FALSE(tt.has_task(initial_last_writer_id));
	...
	TEST_CASE("collective host tasks do not order-depend on their predecessor if it is shadowed by a horizon", "[task_manager][task-graph][task-horizon]") {
		...
		const auto second_collective_deps = tt.get_task(second_collective)->get_dependencies();
	...
	TEST_CASE("buffer accesses with empty ranges do not generate data-flow dependencies", "[task_manager][task-graph]") {
		...
		CHECK(tt.has_any_dependency(read_tid, write_tid) == (!write_empty && !read_empty));
	}
	...
	TEST_CASE("side effects generate appropriate task-dependencies", "[task_manager][task-graph][side-effect]") {
		...
		if(expected_b) {
			CHECK(deps_b.front().node == tt.get_task(tid_a));
	...
	TEST_CASE("side-effect dependencies are correctly subsumed by horizons", "[task_manager][task-graph][task-horizon]") {
		...
		const auto& second_deps = tt.get_task(second_task)->get_dependencies();
	...
	TEST_CASE("epochs create appropriate dependencies to predecessors and successors", "[task_manager][task-graph][epoch]") {
		...
		CHECK(tt.has_dependency(tid_epoch, tid_a));
		CHECK(tt.has_dependency(tid_epoch, tid_b));
		CHECK(tt.has_dependency(tid_c, tid_epoch));
		CHECK_FALSE(tt.has_any_dependency(tid_c, tid_a));
		CHECK(tt.has_dependency(tid_d, tid_epoch)); // needs a true_dep on barrier since it only has anti_deps otherwise
		CHECK_FALSE(tt.has_any_dependency(tid_d, tid_b));
		CHECK(tt.has_dependency(tid_e, tid_epoch));
		CHECK(tt.has_dependency(tid_f, tid_d));
		CHECK_FALSE(tt.has_any_dependency(tid_f, tid_epoch));
		CHECK(tt.has_dependency(tid_g, tid_f, dependency_kind::anti_dep));
		CHECK(tt.has_dependency(tid_g, tid_epoch)); // needs a true_dep on barrier since it only has anti_deps otherwise
	}
	...
	TEST_CASE("fences introduce dependencies on host objects", "[task_manager][task-graph][fence]") {
		...
		CHECK(tt.has_dependency(tid_fence, tid_a));
		CHECK(tt.has_dependency(tid_b, tid_fence));
	}
	...
	TEST_CASE("fences introduce data dependencies", "[task_manager][task-graph][fence]") {
		...
		CHECK(tt.has_dependency(tid_fence, tid_a));
		CHECK(tt.has_dependency(tid_b, tid_fence, dependency_kind::anti_dep));
	}
