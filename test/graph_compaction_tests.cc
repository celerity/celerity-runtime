...
TEST_CASE("reaching an epoch will prune all nodes of the preceding task graph", "[task_manager][task-graph][epoch]") {
	...
	const auto check_task_has_exact_dependencies = [&](const char* info, const task_id dependent,
		...
		INFO(info);
		CAPTURE(dependent);
		const auto actual = tt.get_task(dependent)->get_dependencies();
	...
	CHECK(!tt.has_task(init_tid));
	CHECK(!tt.has_task(writer_tid));
	REQUIRE(tt.has_task(epoch_tid));
	check_task_has_exact_dependencies("epoch after", epoch_tid, {});
	REQUIRE(tt.has_task(reader_writer_tid));
	...
	REQUIRE(tt.has_task(late_writer_tid));
	...
	REQUIRE(tt.has_task(reader_tid));
