...
namespace detail {
	...
	TEST_CASE("horizons prevent number of regions from growing indefinitely", "[horizon][command-graph]") {
		...
		auto buf_a = mbf.create_buffer<2>(range<2>(num_timesteps, full_range.size()));
		...
		for(int timestep = 0; timestep < num_timesteps; ++timestep) {
			const auto read_accessor = [t = timestep, grow = growing_reads](celerity::chunk<1> chnk) {
				...
				ret.range = range<2>(grow ? t : 1, chnk.global_size.get(0));
				ret.offset = id<2>(grow ? 0 : std::max(t - 1, 0), 0);
				return ret;
			};

			const auto latest_write_accessor = [t = timestep](celerity::chunk<1> chnk) {
				celerity::subrange<2> ret;
				ret.range = range<2>(1, chnk.range.size());
				ret.offset = id<2>(t, chnk.offset.get(0));
				return ret;
			};
