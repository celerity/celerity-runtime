...
TEST_CASE("debug names can be set and retrieved from tasks", "[debug]") {
	...
	SECTION("Host Task") {
		...
		CHECK(tt.get_task(tid_a)->get_debug_name() == task_name);
		CHECK(tt.get_task(tid_b)->get_debug_name().empty());
	}
	...
	SECTION("Compute Task") {
		...
		CHECK(tt.get_task(tid_a)->get_debug_name() == task_name);
		CHECK_THAT(tt.get_task(tid_b)->get_debug_name(), Catch::Matchers::ContainsSubstring("compute_task_unnamed"));
	}
	...
	SECTION("ND Range Task") {
		...
		CHECK(tt.get_task(tid_a)->get_debug_name() == task_name);
		CHECK_THAT(tt.get_task(tid_b)->get_debug_name(), Catch::Matchers::ContainsSubstring("nd_range_task_unnamed"));
	}
}
...
TEST_CASE("device_compute tasks derive debug name from kernel name", "[task]") {
	...
	const auto t1 = tt.get_task(tt.tm.submit_command_group([](handler& cgh) { cgh.parallel_for<class MyFirstKernel>(range<1>{1}, [](id<1>) {}); }));
	const auto t2 = tt.get_task(tt.tm.submit_command_group([](handler& cgh) { cgh.parallel_for<foo::MySecondKernel>(range<1>{1}, [](id<1>) {}); }));
	const auto t3 = tt.get_task(tt.tm.submit_command_group([](handler& cgh) { cgh.parallel_for<MyThirdKernel<int>>(range<1>{1}, [](id<1>) {}); }));
