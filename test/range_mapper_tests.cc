...
TEST_CASE("neighborhood built-in range mapper behaves as expected", "[range-mapper]") {
	SECTION("with shape = bounding_box") {
		{
			range_mapper rm{acc::neighborhood<1>(10), access_mode::read, range<1>{128}};
			const auto r = rm.map_1(chunk<1>{{15}, {10}, {128}});
			CHECK(r == box<1>(5, 35));
		}
		{
			range_mapper rm{acc::neighborhood<2>(10, 10), access_mode::read, range<2>{128, 128}};
			const auto r = rm.map_2(chunk<2>{{5, 100}, {10, 20}, {128, 128}});
			CHECK(r == box<2>({0, 90}, {25, 128}));
		}
		{
			range_mapper rm{acc::neighborhood<3>(3, 4, 5), access_mode::read, range<3>{128, 128, 128}};
	...
	SECTION("with shape = along_axes") {
		{
			range_mapper rm{acc::neighborhood(range(10), acc::along_axes), access_mode::read, range<1>{128}};
			const auto r = rm.map_1(chunk<1>{{15}, {10}, {128}});
			CHECK(r == box<1>({5}, {35}));
		}
		{
			range_mapper rm{acc::neighborhood(range(10, 10), acc::along_axes), access_mode::read, range<2>{128, 128}};
			const auto r = rm.map_2(chunk<2>{{5, 100}, {10, 20}, {128, 128}});
			CHECK(r == region<2>({box<2>({0, 100}, {25, 120}), box<2>({5, 90}, {15, 128})}));
		}
		{
			range_mapper rm{acc::neighborhood(range(3, 4, 5), acc::along_axes), access_mode::read, range<3>{128, 128, 128}};
			const auto r = rm.map_3(chunk<3>{{3, 4, 5}, {1, 1, 1}, {128, 128, 128}});
