...
void tiling_calculation(celerity::queue& queue, celerity::buffer<T, 1>& point_buffer, celerity::buffer<Q, 3, compression_tile_type>& tile_points,
	...
	queue.submit([&](celerity::handler& cgh) {
		...
		cgh.parallel_for(celerity::nd_range<3>(range, celerity::range<3>(1, 1, WorkItemsPerTile)), [=](celerity::nd_item<3> item) {
			// auto decompressed_accessor = [&]() -> auto {
			// 	if constexpr(compression_method == celerity::compression_category::element_wise) {
			// 		return tile_point_acc;
			// 	} else {
			// 		return std::move(tile_point_acc.decompress_data(item, tile_point_count_acc, {x_min, y_min, 0}, TileSize));
			// 	}
			// }();
			// auto decompressed_accessor = tile_point_acc.decompress_data(item, tile_point_count_acc, {x_min, y_min, 0}, TileSize);
			auto decompressed_accessor = [&]<celerity::compression_category CM>(auto&& acc, auto&& count_acc, auto&& it) {
				if constexpr(CM == celerity::compression_category::element_wise) {
					return acc; // returns the accessor type
				} else {
					return std::move(acc.decompress_data(it, count_acc, Point{x_min, y_min, 0}, TileSize)); // returns RAII wrapper
				}
			}.template operator()<compression_method>(tile_point_acc, tile_point_count_acc, item);
			...
			for(size_t i = 0; i < num_points_per_runner; i++) {
				...
				if(static_cast<size_t>(pos_x) == item.get_global_id(0) && static_cast<size_t>(pos_y) == item.get_global_id(1)) {
					...
					decompressed_accessor[{item.get_global_id(0), item.get_global_id(1), x}] = point_acc[idx];
				}
			}
		});
	});
