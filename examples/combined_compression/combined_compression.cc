...
int main(int argc, char* argv[]) {
	...
	compression_tile_type compression_tile = [&]<celerity::compression_category CM>() {
		using tile_t = celerity::compressed<celerity::compression::point_cloud<Point, CompressedPoint>, CM>;

		if constexpr(CM == celerity::compression_category::element_wise) {
			Point origin{x_min, y_min, 0};
			return tile_t(origin, tile_size);
		} else if constexpr((CM & (celerity::compression_category::global_memory | celerity::compression_category::local_memory))
		                    != celerity::compression_category::none) {
			return tile_t();
		} else {
			static_assert(CM != celerity::compression_category::none, "Compression category not supported for this function");
		}
	}.template operator()<compression_method>();


	celerity::buffer<Point, 3, compression_tile_type> tile_points({width, height, points_per_tile}, compression_tile);
	celerity::debug::set_buffer_name(tile_points, "tile_points");

	if constexpr(compression_method != celerity::compression_category::element_wise) { tile_points.init(queue); }
	...
	queue.submit([&](celerity::handler& cgh) {
		...
		cgh.host_task(celerity::on_master_node, [=]() {
			auto vec = [&]<celerity::compression_category CM>(auto& acc) {
				if constexpr(CM == celerity::compression_category::element_wise) {
					return acc.get_decompressed_data(tile_points_range);
				} else if constexpr((CM & (celerity::compression_category::global_memory | celerity::compression_category::local_memory))
				                    != celerity::compression_category::none) {
					return acc.decompress_data(width, height, tile_points_count_acc);
				} else {
					static_assert(CM != celerity::compression_category::none, "Compression category not supported for this function");
				}
			}.template operator()<compression_method>(tile_points_acc);
	...
	queue.wait(celerity::experimental::barrier);
	std::cout << "Writing shape factors" << std::endl;
