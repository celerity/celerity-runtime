...
class handler<is_prepass::true_t> {
  public:
	...
	template <cl::sycl::access::mode Mode>
	void require(prepass_accessor<Mode> a, size_t buffer_id, std::unique_ptr<detail::range_mapper_base> rm);
  ...
  private:
	...
	handler(distr_queue& q, size_t task_id) : queue(q), task_id(task_id) { debug_name = (boost::format("task%d") % task_id).str(); }
};
...
class handler<is_prepass::false_t> {
  ...
  private:
	...
	handler(distr_queue& q, size_t task_id, cl::sycl::handler* sycl_handler) : queue(q), task_id(task_id), sycl_handler(sycl_handler) {
