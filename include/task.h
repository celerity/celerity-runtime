...
namespace detail {
	...
	class command_launcher_storage_base {
	  public:
		...
		virtual std::future<host_queue::execution_info> operator()(host_queue& q, const subrange<3>& sr) const = 0;
	...
	class command_launcher_storage : public command_launcher_storage_base {
	  public:
		...
		std::future<host_queue::execution_info> operator()(host_queue& q, const subrange<3>& sr) const override {
			return invoke<std::future<host_queue::execution_info>>(q, sr);
		}
