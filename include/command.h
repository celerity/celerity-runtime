...
namespace detail {
	...
	class task_command : public abstract_command {
	  protected:
		task_command(command_id cid, const task* tsk) : abstract_command(cid), m_task(tsk) {}
	...
	class epoch_command final : public matchbox::implement_acceptor<task_command, epoch_command> {
		friend class command_graph;
		epoch_command(const command_id cid, const task* tsk, const epoch_action action, std::vector<reduction_id> completed_reductions)
	...
	class horizon_command final : public matchbox::implement_acceptor<task_command, horizon_command> {
		friend class command_graph;
		horizon_command(const command_id cid, const task* tsk, std::vector<reduction_id> completed_reductions)
	...
	class execution_command final : public matchbox::implement_acceptor<task_command, execution_command> {
	  ...
	  protected:
		execution_command(const command_id cid, const task* tsk, subrange<3> execution_range) : acceptor_base(cid, tsk), m_execution_range(execution_range) {}
