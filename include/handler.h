...
class handler {
  public:
	...
	template <typename Name, int Dims, typename... ReductionsAndKernel>
	void parallel_for(cl::sycl::range<Dims> global_size, cl::sycl::id<Dims> global_offset, ReductionsAndKernel... reductions_and_kernel) {
		static_assert(sizeof...(reductions_and_kernel) > 0, "No kernel given");
		parallel_for_reductions_and_kernel<Name, Dims, ReductionsAndKernel...>(
...
void handler::parallel_for_kernel_and_reductions(
    cl::sycl::range<Dims> global_size, cl::sycl::id<Dims> global_offset, Kernel& kernel, std::index_sequence<ReductionIndices...>, Reductions&... reductions) {
	if(is_prepass()) {
	...
	device_handler.submit_to_sycl([&](cl::sycl::handler& cgh) {
		cgh.parallel_for<Name>(
		    detail::range_cast<Dims>(sr.range), detail::id_cast<Dims>(sr.offset), detail::make_sycl_reduction(cgh, reductions)..., detail::copy_kernel(kernel));
	});
}
