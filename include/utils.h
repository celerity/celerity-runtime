...
// A parameter pack extension to the implementation from Boost.ContainerHash, licensed under the Boost Software License, Version 1.0.
...
struct vector_hash {
	template <typename U>
	std::size_t operator()(const std::vector<U>& v) const {
		std::size_t seed = 0;
		for(auto& e : v) {
			hash_combine(seed, e);
		}
