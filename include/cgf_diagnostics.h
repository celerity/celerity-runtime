...
class cgf_diagnostics {
  public:
	...
	void check_device_kernel(const Closure& kernel, const buffer_access_map& buffer_accesses) {
		static_assert(std::is_copy_constructible_v<std::decay_t<Closure>>);
		do_check(execution_target::device, kernel, &buffer_accesses, 0);
	}

	template <typename Closure>
	void check_host_kernel(const Closure& kernel, const buffer_access_map& buffer_accesses, const size_t non_void_side_effects_count) {
		static_assert(std::is_copy_constructible_v<std::decay_t<Closure>>);
		do_check(execution_target::host, kernel, &buffer_accesses, non_void_side_effects_count);
	}

	void register_accessor(const hydration_id hid, const execution_target target) {
		if(!m_is_checking) return;
		assert(hid - 1 < m_expected_buffer_accesses->get_num_accesses());
		if(target != m_expected_target) {
			throw std::runtime_error(fmt::format("Accessor {} for buffer {} has wrong target ('{}' instead of '{}').", hid - 1,
			    m_expected_buffer_accesses->get_nth_access(hid - 1).first, target == execution_target::device ? "device" : "host_task",
			    m_expected_target == execution_target::device ? "device" : "host_task"));
		}
	...
	void register_side_effect() {
		if(!m_is_checking) return;
		if(m_expected_target != execution_target::host) { throw std::runtime_error("Side effects can only be used in host tasks."); }
		m_registered_side_effect_count++;
	}

  private:
	inline static thread_local std::unique_ptr<cgf_diagnostics> m_instance;

	bool m_is_checking = false;
	execution_target m_expected_target = execution_target::none;
	...
	template <typename Closure>
	void do_check(
	    const execution_target target, const Closure& kernel, const buffer_access_map* const buffer_accesses, const size_t expected_side_effects_count) {
		m_expected_target = target;
		...
		if(target == execution_target::host) {
