...
#include "split.h"
#include "task.h"
#include "task_manager.h" // TODO for initial_epoch_task, remove


namespace celerity::detail {

command_graph_generator::command_graph_generator(
    const size_t num_nodes, const node_id local_nid, command_graph& cdag, detail::command_recorder* recorder, const policy_set& policy)
...
void command_graph_generator::generate_anti_dependencies(
	...
	for(const auto& [box, wcs] : last_writers) {
		...
		assert(!utils::isa<task_command>(last_writer_cmd) || utils::as<task_command>(last_writer_cmd)->get_task()->get_id() != tid);
		...
		for(auto d : last_writer_cmd->get_dependents()) {
			...
			if(utils::isa<task_command>(cmd) && utils::as<task_command>(cmd)->get_task()->get_id() == tid) continue;
...
void command_graph_generator::generate_epoch_dependencies(abstract_command* cmd) {
	...
	if(const auto deps = cmd->get_dependencies();
		...
		if (!utils::isa<epoch_command>(cmd) || utils::as<epoch_command>(cmd)->get_task()->get_id() != task_manager::initial_epoch_task) {
