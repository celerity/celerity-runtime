...
void normalize_impl(std::vector<box<StorageDims>>& boxes) {
	...
	// (hopefully) fast path: attempt to merge without dissecting first
	boxes.erase(merge_connected_boxes<EffectiveDims>(boxes.begin(), boxes.end()), boxes.end());
	assert(!boxes.empty());
	if(boxes.size() == 1) { return; }

	// 1. dissect boxes along the edges of all other boxes (except the last, "fastest" dim) to create the "maximally mergeable set" of small boxes for step 2
	std::vector<std::vector<size_t>> cuts;
	if constexpr(EffectiveDims > 1) {
		cuts.resize(EffectiveDims - 1);
	...
	auto last = boxes.end();
	if constexpr(EffectiveDims != 1) { // in 1D regions, merge_adjacent will already remove all overlaps
...
void normalize(std::vector<box<Dims>>& boxes) {
	...
	case 0: normalize_impl<0>(boxes); break;
	case 1: if constexpr(Dims >= 1) { normalize_impl<1>(boxes); break; }
...
region<Dims> region_intersection(const region<Dims>& lhs, const region<Dims>& rhs) {
	...
	// clang-format off
	switch(effective_dims) {
	case 0: return grid_detail::region_intersection_impl<0>(lhs, rhs);
	case 1: if constexpr(Dims >= 1) { return grid_detail::region_intersection_impl<1>(lhs, rhs); }
	case 2: if constexpr(Dims >= 2) { return grid_detail::region_intersection_impl<2>(lhs, rhs); }
	case 3: if constexpr(Dims >= 3) { return grid_detail::region_intersection_impl<3>(lhs, rhs); }
	}
	// clang-format on
	abort(); // unreachable
}
...
region<Dims> region_difference(const region<Dims>& lhs, const region<Dims>& rhs) {
	...
	// clang-format off
	switch(effective_dims) {
	case 0: grid_detail::apply_region_difference<0>(dissected_left, rhs); break;
	case 1: if constexpr(Dims >= 1) { grid_detail::apply_region_difference<1>(dissected_left, rhs); break; }
	case 2: if constexpr(Dims >= 2) { grid_detail::apply_region_difference<2>(dissected_left, rhs); break; }
	case 3: if constexpr(Dims >= 3) { grid_detail::apply_region_difference<3>(dissected_left, rhs); break; }
