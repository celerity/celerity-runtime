...
namespace detail {
	...
	void memcpy_strided_device(cl::sycl::queue& queue, const void* source_base_ptr, void* target_base_ptr, size_t elem_size,
		...
		const size_t line_size = elem_size * copy_range[0];
		queue
		    .memcpy(static_cast<char*>(target_base_ptr) + elem_size * get_linear_index(target_range, target_offset),
	...
	void memcpy_strided_device(cl::sycl::queue& queue, const void* source_base_ptr, void* target_base_ptr, size_t elem_size,
		...
		for(size_t i = 0; i < copy_range[0]; ++i) {
			const auto* const source_ptr =
			    static_cast<const char*>(source_base_ptr) + elem_size * (source_base_offset + i * (source_range[1] * source_range[2]));
