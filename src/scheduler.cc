...
class command_queue {
  public:
	bool should_dequeue(const experimental::lookahead lookahead) const {
		...
		if(lookahead == experimental::lookahead::none) return true;                        // unconditionally dequeue, and do not inspect command hints
...
struct scheduler_impl {
	...
	scheduler_impl(bool test_start_idle, size_t num_nodes, node_id local_node_id, const system_info& system, scheduler::delegate* dlg, command_recorder* crec,
...
scheduler_impl::scheduler_impl(const bool test_start_idle, const size_t num_nodes, const node_id local_node_id, const system_info& system,
...
scheduler::scheduler(const size_t num_nodes, const node_id local_node_id, const system_info& system, delegate* const delegate, command_recorder* const crec,
    ...
    : m_impl(std::make_unique<scheduler_impl>(false /* test_start_idle */, num_nodes, local_node_id, system, delegate, crec, irec, policy)) {}
...
scheduler::scheduler(const test_start_idle_tag /* test_start_idle */, const size_t num_nodes, const node_id local_node_id, const system_info& system,
    ...
    : m_impl(std::make_unique<scheduler_impl>(true /* test_start_idle */, num_nodes, local_node_id, system, delegate, crec, irec, policy)) {}
