...
namespace detail {
	...
	abstract_scheduler::abstract_scheduler(const size_t num_nodes, const node_id local_node_id, const system_info& system, 
	    ...
	    : m_cdag(std::make_unique<command_graph>()), m_crec(crec),
	      ...
	      m_iggen(std::make_unique<instruction_graph_generator>(
	          num_nodes, local_node_id, system, *m_idag, delegate, irec, policy.instruction_graph_generator)) {}
	...
	scheduler::scheduler(const size_t num_nodes, const node_id local_node_id, const system_info& system,  delegate* const delegate,
