...
void initialize(const runtime_configuration& cfg) {
	...
	// pinned threads per process: user, scheduler, executor, 1 backend worker per device
	uint32_t pinned_threads_per_process = 3 + g_state.config.num_devices;
	// total number of threads to be pinned across processes (legacy mode)
	uint32_t total_threads = pinned_threads_per_process * g_state.config.num_legacy_processes;

	if(g_state.config.enabled) {
		// build our pinning plan
		if(!cfg.hardcoded_core_ids.empty()) {
			...
			uint32_t core_id = cfg.legacy_process_index * pinned_threads_per_process;
			g_state.thread_pinning_plan.emplace(thread_type::user, cfg.hardcoded_core_ids[core_id++]);
			g_state.thread_pinning_plan.emplace(thread_type::scheduler, cfg.hardcoded_core_ids[core_id++]);
			g_state.thread_pinning_plan.emplace(thread_type::executor, cfg.hardcoded_core_ids[core_id++]);
			for(uint32_t i = 0; i < g_state.config.num_devices; ++i) {
				g_state.thread_pinning_plan.emplace(static_cast<thread_type>(thread_type::first_backend_worker + i), cfg.hardcoded_core_ids[core_id++]);
			}
		} else {
			...
			uint32_t current_core = g_state.config.standard_core_start_id;
			uint32_t assigned_cores = 0;

			// find the next sequential core we may use
			auto next_available_core = [&]() {
				if(!g_state.config.enabled) return 0u;
				while(CPU_ISSET(current_core, &g_state.available_cores) == 0 && current_core < CPU_SETSIZE) {
					current_core++;
				}
			...
			// skip cores used by other processes in legacy mode
			for(uint32_t i = 0; i < g_state.config.legacy_process_index * pinned_threads_per_process; ++i) {
				next_available_core();
			}

			g_state.thread_pinning_plan.emplace(thread_type::user, next_available_core());
			g_state.thread_pinning_plan.emplace(thread_type::scheduler, next_available_core());
			g_state.thread_pinning_plan.emplace(thread_type::executor, next_available_core());
			for(uint32_t i = 0; i < g_state.config.num_devices; ++i) {
				g_state.thread_pinning_plan.emplace(static_cast<thread_type>(thread_type::first_backend_worker + i), next_available_core());
			}
		}
	} else {
...
void pin_this_thread(const thread_type t_type) {
	...
	CELERITY_DEBUG("Affinity: pinned thread of type '{}' to core {} (local process #{} thread id {:x})", //
